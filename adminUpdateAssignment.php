<?php
	...
	if (isset($_POST['uSubmit'])) {
		...
		$stmts = $conn->query("SELECT * FROM undolt.student where sid = '".$row['sid']."';");
		$rows=$stmts->fetch(PDO::FETCH_ASSOC);
		$stmtt = $conn->query("SELECT * FROM undolt.sreferral where sid = '".$row['sid']."';");
...
<body>
	...
	<?php
	  ...
	  <?php  
	  		if($row['assignTid']!=-999){
	  			$stmt2 = $conn->query("SELECT tName,rating FROM undolt.tutor where tid = '".$row['assignTid']."';");
	...
	<form action="adminUpdateAssignmentBack.php" method="post" enctype="multipart/form-data" style="text-align: center;" >
