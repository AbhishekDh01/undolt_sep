<?php  
	...
	session_start();
	$sid=$_SESSION['navSid'];
	$stmt = $conn->query("SELECT * FROM undolt.student where sid=$sid");
	$row=$stmt->fetch(PDO::FETCH_ASSOC);

	$stmt2 = $conn->query("SELECT * FROM undolt.sreferral where sid=$sid");
	$row2=$stmt2->fetch(PDO::FETCH_ASSOC);

	// $stmt2 = $conn->query("SELECT count(*) as currAssign FROM undolt.assignment where assignTid=$tid and aStatus = 'Accepted by Tutor'; ");
	// $stmt3 = $conn->query("SELECT count(*) as reviewAssign FROM undolt.assignment where assignTid=$tid and aStatus = 'Solved and in Review'; ");
	// $stmt4 = $conn->query("SELECT count(*) as comAssign FROM undolt.assignment where assignTid=$tid and (aStatus = 'Completed' or aStatus = 'Reviewed' ); ");
