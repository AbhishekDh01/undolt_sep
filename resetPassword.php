<?php  
	...
	if(isset($_GET['id'])){
		...
		if ($_GET['id']=='sid') {
			$stmt = $conn->query("SELECT sid FROM undolt.student where vKey='".$_GET['vKey']."'");
			try{
			...
			$stmt = $conn->query("SELECT tid FROM undolt.tutor where vKey='".$_GET['vKey']."'");
			$row = $stmt->fetch(PDO::FETCH_ASSOC);
			$who = 'tutor';
			$id = $row['tid']??-999;
			...
			$stmt = $conn->query("SELECT mid FROM undolt.manager where vKey='".$_GET['vKey']."'");
			$row = $stmt->fetch(PDO::FETCH_ASSOC);
			$who = 'manager';
			$id = $row['mid']??-999;
...
<?php  
	if(isset($_POST['reset'])){
		if ($_POST['who']=='student') {
			...
			$pass = md5($_POST['pass']);
			echo $sid;
			$sql1 = "update undolt.student set sPass = '$pass' where sid = $sid;";
			$sql2 = "update undolt.student set vKey = 'XXX' where sid = $sid;";
		...
		if ($_POST['who']=='tutor') {
			$tid = $_POST['id'];
			$pass = md5($_POST['pass']);
			$sql1 = "update undolt.tutor set tPass = '$pass' where tid = $tid;";
			$sql2 = "update undolt.tutor set vKey = 'XXX' where tid = $tid;";
			$stmt1 = $conn->prepare($sql1);
			$stmt2 = $conn->prepare($sql2);
		...
		if ($_POST['who']=='manager') {
			$mid = $_POST['id'];
			$pass = md5($_POST['pass']);
			$sql1 = "update undolt.manager set mPass = '$pass' where mid = $mid;";
			$sql2 = "update undolt.manager set vKey = 'XXX' where mid = $mid;";
			$stmt1 = $conn->prepare($sql1);
			$stmt2 = $conn->prepare($sql2);
