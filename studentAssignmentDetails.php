...
<body>
	...
	<?php
		if($row['approvedByMid']!=-999){
			$stmt2 = $conn->query("SELECT mContact FROM undolt.manager where mid = '".$row['approvedByMid']."';");
		...
		if(isset($_POST['rSubmit']) && $row['aStatus']!='Reviewed'){
			$stmt0 = $conn->query("SELECT count(*) as comAssign FROM undolt.assignment where assignTid=$tid and (aStatus = 'Completed' or aStatus = 'Reviewed' ); ");
			$row0=$stmt0->fetch(PDO::FETCH_ASSOC);
			$no = $row0['comAssign'];
			
			$rating = $_POST['rating'];
			$sql1 = "update undolt.assignment set aStatus = 'Reviewed' where qid = $qid;";
			

			$sql2 = "update undolt.tutor set rating = ceil((rating*($no-1)+$rating)/$no) where tid = $tid;";
