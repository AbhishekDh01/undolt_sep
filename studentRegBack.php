...
if(isset($_POST['sName'])){
	...
	$stmt0 = $conn->query("SELECT sid FROM undolt.sreferral where ref_code = '".$sRef."';");
	...
	if(!isset($row0['sid'])){
		...
		$rSid = $row0['sid'];
		$sql99 = "update undolt.sreferral set total_ref = total_ref+1 where sid = $rSid;";
	...
	$sql = "INSERT INTO undolt.student(sName,sEmail,sContact,sPass,vkey) VALUES(:n,:e,:c,:p,:v)";
	$pass = md5($_POST['sPass']);
	$stmt = $conn->prepare($sql);

	$sql2 = "INSERT INTO undolt.sreferral(sid,ref_code,referral_sid) VALUES(:s,:r,:rs)";
