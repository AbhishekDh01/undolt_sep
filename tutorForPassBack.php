...
if(isset($_POST['tForget'])){
	...
	$sql = "SELECT banned,verified from undolt.tutor where tEmail = :e";
		...
		if($stmt->rowCount()>0){
			$getRow = $stmt->fetch(PDO::FETCH_ASSOC);						
						...
						if($getRow['verified']==0){
							...
							verificationEmail('no_reply@undolt.com','tid',$tEmail,$vKey,'https://oneshopy.in/verifyAccount.php');
							header("refresh:0; url= index.html");
							exit();
						}
						
						echo $vKey;
						$sql2 = "update undolt.tutor set vKey = '$vKey' where tEmail = '".$tEmail."';";

						$stmt2 = $conn->prepare($sql2);

						try{
							$stmt2->execute();
							resetEmail('no_reply@undolt.com','tid',$tEmail,$vKey,'https://oneshopy.in/resetPassword.php');
